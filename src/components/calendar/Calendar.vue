<template>
  <b-container>
    ...
    <div class="p-2" style="border: 2px black solid; width: fit-content; overflow: hidden">
      <b-row
        style="padding: 15px; margin-right: 20px; width: fit-content; overflow-wrap: break-word"
      >
        <b-col class="row" style="justify-content: space-between">
          <div>From <input v-model="fromDate" type="date" style="margin-right: 30px" /></div>
          <div>
            To
    ...
    <div class="p-2">
      <div class="nav justify-content-end">
        <button
          @click="save()"
          :class="{ 'nav-link btn btn-outline-danger rounded-0': true, disabled: !getterIsSave }"
          style="max-width: fit-content; float: left; padding: 10px 30px; font-weight: 600"
        >
          Save Changes
        </button>
      </div>
